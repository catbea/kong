...
export { onSendMsg, initMsg, callbackaddMsg, setToAccount, webimLogin, callbackaddMsgCount, callbackDynamics }
...
function onMsgNotify(newMsgList) {
  ...
  for (var j in newMsgList) {
    ...
    if (newMsg.getSession().id() == toAccount) {
      ...
      if (content.desc == 1 || content.desc == 2 || content.desc == 3 || content.desc == 6 || content.desc == 7) {
...
function onSendMsg(msgtosend, isSend, msgType, audioTime, user = {}) {
  ...
  webim.sendMsg(
    ...
    function(resp) {
      ...
      if (resp.ActionStatus == 'OK' && elems.content.desc != 4) {
